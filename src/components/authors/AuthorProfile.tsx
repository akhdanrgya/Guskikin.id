...
export function AuthorProfile({ author, className = '', compact = false }: AuthorProfileProps) {
  ...
  if (compact) {
    return (
      <div className={`flex min-w-0 items-center gap-2.5 ${className}`}>
        <span className="relative grid size-9 shrink-0 place-items-center overflow-hidden rounded-full bg-primary font-label-sm text-label-sm font-bold text-primary-foreground">
          {avatar?.url ? (
            <Image
        ...
        <span className="min-w-0">
          <span className="block truncate font-label-sm text-label-sm font-bold text-primary">{name}</span>
          <span className="block line-clamp-1 font-caption text-caption text-text-body">{bio}</span>
        </span>
      </div>
    )
  }
